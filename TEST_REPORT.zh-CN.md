...
- 关闭局域网访问；AI 与其他全球 DNS 显式走 `PROXY`，私有、中国、Apple CN、一般 Microsoft/OneDrive 与节点 DNS 显式走 `DIRECT`。AI DNS 策略位于 Microsoft/OneDrive 之前，Copilot 等重叠域名仍走 `PROXY`。
- `PROXY` 组不再提供 `DIRECT`，防止手动选择后让标记为 `#PROXY` 的 DNS 实际直连。
...
- Parsec 的 Windows/macOS/Linux 进程入口均进入专用子规则：私有与中国大陆对端 IP 走 `DIRECT`，其余走 `PROXY`；仅 STUN 端点保留无条件直连。
...
- Full 配置中的远程 MRS（含新增的 `category-ai-cn`）已经由 `gh-proxy.org` 实际下载，并逐个交给 Mihomo 解码；测试同时统计远程 domain provider 的精确重叠项，避免仅验证 HTTP 状态或文件扩展名。
- `tests/cases.yaml` 的域名案例会在远程 MRS 解码后按完整规则顺序检查第一命中，避免只确认 provider 存在而产生假通过；Apple CN、游戏平台、中国域名与 Parsec STUN 的广泛/专属 provider 顺序均已纳入验证。
...
- 本机隔离 DNS 端点已确认一般 Microsoft/OneDrive 查询只命中直连 DNS，Copilot 重叠域名与默认全球查询只命中代理 DNS。
...
macOS 本机的隔离代理、provider 和 IPv6 回环路径已自动验证，但这不等同于公网纯 IPv6、TUN 或其他操作系统。Windows/Linux TUN、Parsec 中国/境外真实对端、运营商公网 IPv6、系统 DNS、WebRTC、休眠切网、防火墙、真实吞吐与资源占用仍需执行 `tests/CROSS_PLATFORM_TESTS.zh-CN.md` 的设备矩阵；任何平台超过 10% 中位数退化或发生泄漏均不得发布。
